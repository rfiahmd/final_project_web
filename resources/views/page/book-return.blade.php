...
@section('content')
                        ...
                        <div class="col-sm-10">
                            <select name="book_id" id="book" class="form-select inputbox">
                                ...
                                @foreach ($books as $item)
                                    <option value="{{ $item->id }}">{{ Str::ucfirst($item->book_code) }}-{{ Str::ucfirst($item->title) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    @if (Session::get('status') == 'failed')
        <div class="bs-toast toast fade show @if (Session::get('message') == 'Buku Berhasil Di Kembalikan')
        bg-success
        @else
        bg-danger
